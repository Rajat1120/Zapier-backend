...
export async function getTokenForUser(userId: number) {
  ...
  if (token.expiresAt <= expiresThreshold) {
    console.log(`🔄 Token for user ${userId} is expired or expires soon, refreshing...`);
    ...
    try {
      ...
      const newExpiresAt = new Date(Date.now() + expires_in * 1000 + istOffset);
  ...
  if (Array.isArray(token.scopes)) {
    ...
    try {
      ...
      console.error('Failed to parse scopes JSON:', error, 'Raw scopes:', token.scopes);
  ...
  console.log(`🔑 Retrieved ${parsedScopes.length} scopes for user ${userId}:`, parsedScopes);
